...
def _encode(config, folder_path, mode='finetune'):
    ...
    if mode != "test" and len(config["encoder"]) != 0:
        ...
        for dataset_name in os.listdir(test_path):
            ...
            encoder_output = os.path.join(folder_path, "encoded/test_{}.bpe.{}".format(dataset_name, config['source_lang_flores200'])) + ' ' + \
                os.path.join(folder_path, "encoded/test_{}.bpe.{}".format(dataset_name, config['target_lang_flores200']))
...
def _preprocess(config, folder_path):
    ...
    if config['perform_finetune'] == True:
        ...
        c = "fairseq-preprocess --source-lang {} --target-lang {} \
            ...
            --srcdict {} --tgtdict {}"
        command = (c.format(config['source_lang_flores200'], config['target_lang_flores200'],
...
def _finetune(config, folder_path):
    # See README.md for full how architecture is obtained
    command = f"fairseq-train {os.path.join(folder_path, config['preprocess']['destdir'])}\
        --source-lang {config['source_lang_flores200']} \
        --target-lang {config['target_lang_flores200']} \
        --arch transformer \
        ...
        --langs {config['finetune']['full_languages']} \
        --lang-pairs {config['source_lang_flores200']}-{config['target_lang_flores200']} \
        --encoder-langtok tgt \
...
def _generate_hypothesis(config, folder_path):
    ...
    for i in range(len(list_output_path)):
        ...
        command = (c.format(
            ...
            config['source_lang_flores200'], config['target_lang_flores200'],
            model_path,
            os.path.join(folder_path, config['preprocess']['dictionary']),
            config['source_lang_flores200'], config['target_lang_flores200'],
            "" if i == 0 else str(i),
...
# Run M2M100 while checking whether fine-tuned or 
def run_nllb(config_path, stats_path):
    logging.debug("=============================")
...
if __name__ == '__main__':
    ...
    run_nllb(config_path=args.config, stats_path=args.stats)