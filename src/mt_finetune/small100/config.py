...
SMALL100_SPM_MODEL = os.path.join(CUR_DIR, "model", "spm.128k")
PRETRAINED_SMALL100_MODEL = os.path.join(CUR_DIR, "model", "model_small100_fairseq.pt")
DEFAULT_DATA_DICT = os.path.join(CUR_DIR, "model", "data_dict.128k.txt")
SMALL100_DEFAULT_DICT = os.path.join(CUR_DIR, "model", "model_dict.128k.txt")
SMALL100_LANGUAGE_PAIR_MODEL = os.path.join(CUR_DIR, "model", "language_pairs_small_models.txt")
SMALL100_LANGUAGE_CODE_MAPPING = os.path.join(CUR_DIR, "model", "language_mapping.csv")
CONFIG_SMALL100_PATH = os.path.join(CUR_DIR, "model", "config_small100.json")
...
def _setup_language_pairs_and_dict(config, folder_path):
    ...
    with open(SMALL100_LANGUAGE_PAIR_MODEL, 'r') as lang_pair_file, \
         open(SMALL100_DEFAULT_DICT, 'r') as model_dict_file:
        pair_content = lang_pair_file.read()
        model_dict_content = model_dict_file.readlines()

    lang_map_df = pd.read_csv(SMALL100_LANGUAGE_CODE_MAPPING)
    small100_src_lang = lang_map_df[lang_map_df['flores_abbr'] == config['source_lang']]['small100_abbr'].unique()[0]
    small100_tgt_lang = lang_map_df[lang_map_df['flores_abbr'] == config['target_lang']]['small100_abbr'].unique()[0]

    # Modify the content based on the flores language abbreviation
    if not pd.isna(small100_src_lang):
        pair_content = pair_content.replace(small100_src_lang, config['source_lang'])
    if not pd.isna(small100_tgt_lang):
        pair_content = pair_content.replace(small100_tgt_lang, config['target_lang'])
    ...
    if config['source_lang'] in LANGUAGES_NOT_PRETRAINED:
        ...
        for i in range(START_LINE_LANG_DICT, len(model_dict_content)):
            if model_dict_content[i] == f"__{small100_src_lang}__ 1\n":    
    ...
    if config['target_lang'] in LANGUAGES_NOT_PRETRAINED:
        ...
        for i in range(START_LINE_LANG_DICT, len(model_dict_content)):
            if model_dict_content[i] == f"__{small100_tgt_lang}__ 1\n":    
...
def _setup_config_finetune_model(config):
    with open(CONFIG_SMALL100_PATH, 'r') as f:
...
def create_config_small100(src_lang, tgt_lang, dataset, size, perform_finetune):
    config = {}
    ...
    if perform_finetune:
        folder_name = "_".join(["small100", src_lang, tgt_lang, dataset, str(size)])
    else:
        folder_name = f"small100_{src_lang}_{tgt_lang}_no_finetune"
    config['folder_name'] = folder_name
    folder_path = os.path.join(EXP_DIR, folder_name)
    
    # Sentencepiece model
    config['sentencepiece'] = {}
    config['sentencepiece']['model_save_path'] = SMALL100_SPM_MODEL
    ...
    # Finetune
    config['finetune'] = {}
    config['finetune']['pretrained_model_path'] = PRETRAINED_SMALL100_MODEL
...
if __name__ == '__main__':
    ...
    create_config_small100(src_lang=args.src_lang, tgt_lang=args.tgt_lang,
