...
from .config import create_config_nllb
from .finetune_nllb import run_nllb
...
def main(src_lang, tgt_lang, dataset, size, perform_finetune):
    if perform_finetune:
        exp_dir_name = f"nllb_{src_lang}_{tgt_lang}_{dataset}_{size}"
    else:
        exp_dir_name = f"nllb_{src_lang}_{tgt_lang}_no_finetune"
    ...
    # Transform dataset
    if perform_finetune:
        transform_dataset(source_language=src_lang, target_language=other_lang, train_size=size, data_type="txt")

    # Configure
    create_config_nllb(src_lang=src_lang, tgt_lang=tgt_lang, dataset=dataset,
                           size=size, perform_finetune=perform_finetune)

    # Finetune if required
    run_nllb(config_path=f"{EXP_DIR}/{exp_dir_name}/{exp_dir_name}.json",
