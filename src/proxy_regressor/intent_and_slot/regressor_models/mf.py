...
class MFRegressor():
    ...
    def predict(self, test_data):
        ...
        for index, record in test_data.iterrows():
            ...
            if f"{src_lang}_{tgt_lang}" in self.context_dict.keys():
                cur_tuple.append(self.context_dict[f"{src_lang}_{tgt_lang}"])
            else:
                raise KeyError
...
class MFPipeline(GenericModelPipeline):
    def __init__(self, model_name, score_name, mf_json, **kwargs):
        ...
        self.score_col_name = f"{model_name}_{score_name}"
    ...
    def get_context_dict_info(self, df, context_features):
        ...
        for index, record in df.iterrows():
            src_lang = record["source_lang"]
            tgt_lang = record["target_lang"]
            side_dict[f"{src_lang}_{tgt_lang}"] = scaled_df.loc[index, context_features].values
        return side_dict
