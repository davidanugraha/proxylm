...
def get_test_outputs(experiment_dir):
    ...
    entries = []
    tmp_entries_dict = {}
    logging.debug(f"start processing {experiment_dir}")
    for exp_subdir in os.listdir(experiment_dir):
        exp_subdir_path = os.path.join(experiment_dir, exp_subdir)
    ...
    logging.debug(f"processed {len(os.listdir(experiment_dir))} experiments")
    return entries
