...
from .config import create_config_m2m100
from .finetune_m2m100 import run_m2m100
...
def main(src_lang, tgt_lang, dataset, size, perform_finetune):
    if perform_finetune:
        exp_dir_name = f"m2m100_{src_lang}_{tgt_lang}_{dataset}_{size}"
    else:
        exp_dir_name = f"m2m100_{src_lang}_{tgt_lang}_no_finetune"
    ...
    # Transform dataset
    if perform_finetune:
        transform_dataset(source_language=src_lang, target_language=tgt_lang, train_dataset=dataset, train_size=size, data_type="txt")

    # Configure
    create_config_m2m100(src_lang=src_lang, tgt_lang=tgt_lang, dataset=dataset,
                           size=size, perform_finetune=perform_finetune)

    # Finetune if required
    run_m2m100(config_path=f"{EXP_DIR}/{exp_dir_name}/{exp_dir_name}.json",
