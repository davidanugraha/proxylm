...
def _convert_train_dataset_from_txt(source_language, target_language, train_dataset, train_size):
    other_lang = source_language if source_language != "eng" else target_language
    for category in CATEGORIES:
        if category == "train":
            ...
            directory = os.path.join(DATASET_FOLDER_PATH, "{}/{}".format(other_lang, category))
...
def transform_dataset(source_language, target_language, train_dataset, train_size, data_type):
    if data_type == "txt":
        ...
        logging.error("Data type is not recognized!")
        raise
